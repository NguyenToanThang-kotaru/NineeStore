...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function statisticProduct(orders) {
        ...
        productSaleList.forEach((product) => {
            ...
            bodyContent +=
                `<tr>
                    ...
                    <td><span class="product-tke-price-value">${(productPrice * product.Quantity).toLocaleString("de-DE")}</span><sup>₫</sup></td>
                    <td>
      ...
      <tr>
                                      ...
                                      <td id="best-item-revenue"><span class="product-tke-price-value">${productSaleList[0].Quantity
            }</span></td>
                                  </tr>
                                  <tr>
    ...
    function statisticCustomer(orders) {
        ...
        // Lấy thông tin khách hàng và tổng giá
        orders.forEach(order => {
            const customerStat = {
                Product: order.ProductList,
                Customer: order.Customer,
                totalPrice: parseFloat(order.TotalPrice.replace(/\./g, '')), // Chuyển đổi chuỗi thành số
                date: new Date(order.OrderDate).toISOString().split('T')[0] // Lấy ngày theo định dạng YYYY-MM-DD
            };

            customerStatArray.push(customerStat);
        });
        let productCustomer = ""
        let insertCus = ""
                            ...
                            <div class="hoa-don-container" id="hoadon-customers">
                              ...
                              <div class="hoa-don-info">
                                  <div class="hoa-don-customer">
                                      ...
                                      <p>Họ tên: ${Customer.Customer.FullName}</p>
                                      <p>Địa chỉ:${Customer.Customer.Address}</p>
